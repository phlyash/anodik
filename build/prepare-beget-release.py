#!/usr/bin/env python3
"""Build the Anodik incoming bundle consumed by the restricted publisher."""
...
RUN_ID_PATTERN = re.compile(r"^[1-9][0-9]*-[1-9][0-9]*$")
FILES = {
...
def validate_input(source):
    ...
    if set(entries) != set(FILES):
        raise ValueError("input must contain exactly the six Anodik archives")

    for name in FILES:
        path = entries[name]
        if path.is_symlink() or not path.is_file():
            raise ValueError(
                "input archive must be a regular non-symlink file: " + name
            )
