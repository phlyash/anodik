...
printf '%s %s %s %s %s %s\n' \
  "$(stat -c '%a' "$key_path")" "$key_path" \
  "$(stat -c '%a' "$known_hosts_path")" "$known_hosts_path" \
  "$(stat -c '%a' "$credential_dir")" "$credential_dir" >>"$MODES"
EOF
...
for bad in \
  ...
  'BEGET_PORT=65536' \
  'BEGET_PORT=22;2' \
  'GITHUB_REPOSITORY=someone/anodik' \
  'GITHUB_REPOSITORY=phlyash/not-anodik' \
  'GITHUB_RUN_ID=0' \
  'GITHUB_RUN_ID=12/3' \
...
mv "$TMP/release.json" "$BUNDLE/release.json"
ln -s "$BUNDLE" "$TMP/bundle-link"
expect_failure run_clean "${base_env[@]}" "$DEPLOY" "$TMP/bundle-link"
...
output=$(run_clean "${base_env[@]}" "$DEPLOY" "$BUNDLE" 2>&1) ||
  fail "valid deployment failed: $output"

[[ $(<"$ATTACKER_KEY") == 'attacker key sentinel' ]] ||
  fail 'preseeded key symlink was overwritten'
[[ $(<"$ATTACKER_KNOWN_HOSTS") == 'attacker known-hosts sentinel' ]] ||
  fail 'preseeded known-hosts symlink was overwritten'
...
while read -r key_mode key_path hosts_mode hosts_path directory_mode credential_dir; do
  [[ $key_mode == 600 && $hosts_mode == 600 ]] ||
    fail 'credential files are not mode 600 during transport'
  [[ $directory_mode == 700 ]] || fail 'credential directory is not mode 700'
  [[ $key_path == "$RUNNER_TEMP"/aspect-ssh.*/* ]] ||
    fail 'credential path is predictable'
  [[ $hosts_path == "$RUNNER_TEMP"/aspect-ssh.*/* ]] ||
    fail 'known-hosts path is predictable'
  [[ ! -e $key_path && ! -e $hosts_path ]] ||
    fail 'credential files were not removed after transport'
done <"$MODES"
...
[[ ${calls[0]} == "ssh $expected_ssh_options baglayt2_aspect@example.invalid aspect-publisher\\ prepare\\ 123-2 " ]] ||
  fail "unexpected prepare call: ${calls[0]}"
[[ ${calls[2]} == "ssh $expected_ssh_options baglayt2_aspect@example.invalid aspect-publisher\\ publish\\ 123-2 " ]] ||
  fail "unexpected publish call: ${calls[2]}"
...
read -r failed_key_mode failed_key_path failed_hosts_mode failed_hosts_path \
  failed_directory_mode failed_credential_dir <"$MODES"
[[ $failed_key_mode == 600 && $failed_hosts_mode == 600 ]] ||
  fail 'failed transport credentials are not mode 600 during transport'
[[ $failed_directory_mode == 700 ]] ||
  fail 'failed transport credential directory is not mode 700'
[[ ! -e $failed_key_path && ! -e $failed_hosts_path && ! -e $failed_credential_dir ]] ||
  fail 'credential directory was not removed after failed transport'
...
: >"$CAPTURE"
: >"$MODES"
(cd "$TMP" && run_clean "${base_env[@]}" "$DEPLOY" '-bundle:host') ||
  fail 'relative hostile bundle deployment failed'
mapfile -t hostile_calls <"$CAPTURE"
[[ ${#hostile_calls[@]} == 3 ]] ||
  fail "expected three hostile transport calls, got ${#hostile_calls[@]}"
HOSTILE_ABSOLUTE=$(realpath -- "$HOSTILE_BUNDLE")
for file in "${sorted_files[@]}"; do
  [[ ${hostile_calls[1]} == *" $HOSTILE_ABSOLUTE/$file "* ]] ||
    fail "scp local path is not canonical and absolute: $file"
done
[[ ${hostile_calls[1]} != *' -bundle:host/'* ]] ||
  fail 'scp received a hostile relative local path'

printf 'PASS: deploy Aspect transport contract\n'
